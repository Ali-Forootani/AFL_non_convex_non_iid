...
# -*- coding: utf-8 -*-
"""
Asynchronous Federated Learning — Dataset switch (Fashion-MNIST / SVHN / EMNIST)
- Algorithm unchanged (same async per-round client training + FedAvg aggregation).
...
def partition_non_iid(dataset, num_clients, num_classes, alpha=0.5):
    ...
    for idx in range(len(dataset)):
        _, label = dataset[idx]
        # SVHN labels might be np.int64; cast to int
        label = int(label)
...
async def train_random_clients_with_classification_objective_delay(
    client_model, train_loader, device, local_epochs, loss_fn, gamma_0, alpha, delay_t=0,
    ...
    for epoch in range(local_epochs):
        ...
        for batch_idx, (inputs, targets) in enumerate(train_loader):
            # SVHN returns PIL -> Tensor, targets int; EMNIST/FashionMNIST same
            inputs = inputs.to(device)
            # SVHN labels may be ndarray; convert
...
# -------------------------
# Federated loop (algorithm unchanged)
# -------------------------
async def federated_learning_with_cnn(
    clients_models, server_model, clients_train_loaders, test_loader,
    num_rounds=10, local_epochs=1, max_clients_per_round=3,
    loss_fn=None, gamma_0=1e-3, alpha=0.1, delay_t=2, accumulation_steps=1,
    early_stopping_patience=10
):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    training_losses = [[] for _ in range(len(clients_models))]
    server_losses = []                            # proxy: mean of client losses this round
    ...
    for round_num in tqdm(range(num_rounds), desc="Federated Rounds"):
        ...
        results = await asyncio.gather(*[train_client_task(i) for i in selected_clients])

        round_loss = 0.0
        exec_times = []
        for i, (state_dict, client_loss, exec_time) in zip(selected_clients, results):

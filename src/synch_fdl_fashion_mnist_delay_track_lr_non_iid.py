...
# -*- coding: utf-8 -*-
"""
Created on Fri Sep  5 12:15:01 2025
...
# -*- coding: utf-8 -*-
"""
Created on Fri Sep  5 11:18:19 2025

@author: forootan

Synchronous Federated Learning — Dataset switch (Fashion-MNIST / SVHN / EMNIST)
- Standard FedAvg with synchronous rounds.
- Per round: broadcast server weights -> selected clients train locally -> aggregate -> evaluate.
- Only dataset & model input dims/classes adapt.
- Logs & saves: client losses, server loss, test accuracy/loss per round,
  communication overhead per round, per-client exec times, round wall-clock times,
  selected clients per round.

Set DATASET to one of {"fashion_mnist", "svhn", "emnist_byclass"}.
Default: "fashion_mnist" (famous dataset, *not* CIFAR-10).
"""
...
gamma_0 = 1e-3              # base LR at clients
...
# Utilities (IO/plots)
# -------------------------
def create_directory(num_clients, num_rounds, local_epochs, max_clients_per_round, dataset_name, base_dir="results"):
...
def save_comm_overhead(comm_rows, filename):
    # comm_rows: list of dicts with keys: round, num_clients, bytes_down, bytes_up, bytes_total, mb_total
    with open(filename, "w") as f:
...
def partition_non_iid(dataset, num_clients, num_classes, alpha=0.5):
    ...
    # robust: EMNIST/SVHN/FashionMNIST all return (img, label)
    ...
    for c in range(num_classes):
        ...
        for i, ct in enumerate(counts):
            ...
            start = end
        # leftover due to rounding -> random clients
...
# -------------------------
# Client local training (synchronous)
# -------------------------
def train_one_client(
    client_model, train_loader, device, local_epochs, loss_fn, base_lr,
    ...
    # Fresh optimizer each round/start
    optimizer = torch.optim.Adam(client_model.parameters(), lr=base_lr)
    ...
    for epoch in range(local_epochs):
        ...
        # simple early stopping on local loss
...
# -------------------------
# Federated loop (synchronous FedAvg)
# -------------------------
def federated_learning_with_cnn(
    clients_models, server_model, clients_train_loaders, test_loader,
    num_rounds=10, local_epochs=1, max_clients_per_round=3,
    loss_fn=None, base_lr=1e-3, accumulation_steps=1,
    early_stopping_patience=10
):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    training_losses = [[] for _ in range(len(clients_models))]
    server_losses = []                            # proxy: mean of selected clients' avg losses per round
    selected_clients_per_round = []               # list[list[int]]
    execution_times_by_round = []                 # list[list[float]]
    round_durations = []                          # list[float]
    comm_stats = []                               

    test_losses_per_round = []                    # saved
    accuracy_per_round = []                       # saved

    # Static per-round model bytes (upload/download) — all clients share same architecture
    model_bytes = state_dict_nbytes(server_model.state_dict())

    for round_num in tqdm(range(num_rounds), desc="Federated Rounds"):
        round_start = time.time()

        # Sample clients for this synchronous round (without replacement)
        ...
        # Broadcast: load server weights into each selected client before local training
        server_state = {k: v.detach().clone() for k, v in server_model.state_dict().items()}
        client_weights = []
        round_loss = 0.0
        exec_times = []

        for i in selected_clients:
            clients_models[i].load_state_dict(server_state, strict=True)

            t0 = time.time()
            state_dict, client_loss = train_one_client(
                client_model=clients_models[i],
        ...
        server_losses.append(round_loss / max(1, len(selected_clients)))
        ...
        new_server_state_dict = {k: torch.zeros_like(v) for k, v in client_weights[0].items()}
        ...
        # -------------------------
        bytes_down = model_bytes * len(selected_clients)  # broadcast current server model to selected clients
        bytes_up   = model_bytes * len(selected_clients)  # each selected client uploads updated weights once
...
def main():
    ...
    # Run federated training (synchronous)
    (server_model, training_losses, server_losses, selected_clients_per_round,
     execution_times_by_round, round_durations, test_losses_per_round,
     accuracy_per_round, comm_stats) = federated_learning_with_cnn(
    ...
    # Save per-client losses + plots
    for i, client_losses in enumerate(training_losses):
        npy_path = os.path.join(results_dir, f"client_{i}_losses.npy")
        save_losses_as_numpy(client_losses, npy_path)
        plot_series(client_losses, title=f"Client {i} Training Losses",
                    xlabel="Epochs (accumulated over rounds)", ylabel="Loss",
                    save_path=os.path.join(results_dir, f"client_{i}_training_loss.png"))

    # Save server loss + plot
    save_losses_as_numpy(server_losses, os.path.join(results_dir, "server_losses.npy"))
    plot_series(server_losses, title="Server Proxy Loss Across Rounds",
                xlabel="Round", ylabel="Loss",
                save_path=os.path.join(results_dir, "server_training_loss.png"))

    # Save accuracy & test loss per round + plots
    ...
    plot_series(accuracy_per_round, title="Test Accuracy per Round",
                xlabel="Round", ylabel="Accuracy",
                save_path=os.path.join(results_dir, "accuracy_plot.png"))
    plot_series(test_losses_per_round, title="Test Loss per Round",
                xlabel="Round", ylabel="Loss",
                save_path=os.path.join(results_dir, "test_loss_plot.png"))

    # Save selected clients & plots
    save_selected_clients(selected_clients_per_round, os.path.join(results_dir, "selected_clients.csv"))
    # quick scatter for selected clients
    ...
    summary = {
        ...
        "accuracy_final": accuracy_per_round[-1] if accuracy_per_round else None,
        "best_accuracy": max(accuracy_per_round) if accuracy_per_round else None,
        "total_runtime_sec": float(sum(round_durations)),
    }
    with open(os.path.join(results_dir, "summary.json"), "w") as f:
        json.dump(summary, f, indent=2)

    print(f"[OK] Results saved in: {results_dir}")
